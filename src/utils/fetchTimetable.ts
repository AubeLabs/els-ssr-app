...
export async function fetchTimetable(date: string, year: string, grade: string, classNm: string): Promise<TimetableEntry[]> {
//   const apiKey = process.env.NEIS_API_KEY
  // const apiUrl = 'http://localhost:8000'
  const apiUrl = 'http://fastapi-nlb-55a77074abb440ea.elb.ap-northeast-2.amazonaws.com'

  const response = await fetch(
    `${apiUrl}/api/els-timetable/B10/7021105/${year}/${grade}/${classNm}/${date}`
  )
