  ...
  return (
    <div className="max-w-7xl mx-auto">
      <h2 className="text-xl font-semibold mb-4">
        서울이문초등학교 {grade}학년 {classNm}반<br></br>(
        {formatDate(currentDate ? currentDate.date : "")})
      </h2>
          ...
          {timetable.length ? (
            <ul>
              {timetable.map((entry, index) => (
                ...
                >
                  {entry.PERIO} 교시 :{" "}
                  {entry.ITRT_CNTNT ? entry.ITRT_CNTNT : "No data"}
                </li>
              ))}
            </ul>
          ) : (
          ...
          {meals.length ? (
            <>
              {meals.map((entry, index) => (
                <table
                  key={index}
                  className="min-w-full bg-white rounded-lg shadow-md mb-4 text-sm"
                >
                  <tbody>
              ...
              <div className="text-xs">
                ...
                <br />- 알레르기 유발 식재료 번호 : 1.난류, 2.우유, 3.메밀,
                4.땅콩, 5.대두, 6.밀, 7.고등어, 8.게, 9.새우, 10.돼지고기,
